//
//  WPMediaRequestFiltersTest.swift
//  WordPressSimplify_Tests
//
//  Created by Diego Badaracco on 09/10/2022.
//  Copyright © 2022 CocoaPods. All rights reserved.
//

import XCTest
@testable import WordPressSimplify

final class WPMediaRequestFiltersTest: XCTestCase {

    override func setUpWithError() throws {
        // Put setup code here. This method is called before the invocation of each test method in the class.
    }

    override func tearDownWithError() throws {
        // Put teardown code here. This method is called after the invocation of each test method in the class.
    }

    func testListMedia() throws {
        let filters: [WPRequestFilter.ListMedia] = [
            .context(type: .view),
            .page(number: 1),
            .perPage(number: 1),
            .search(value: "pepe"),
            .after(dateISO8601: "after_1"),
            .author(id: 1),
            .authorExclude(ids: [1,2]),
            .before(dateISO8601: "before_1"),
            .exclude(ids: [3,4]),
            .include(ids: [5,6]),
            .offset(number: 7),
            .order(type: .asc),
            .orderBy(type: .id),
            .parent(id: 1),
            .parentExclude(ids: [2,3]),
            .slug(slugs: ["slug1"]),
            .status(value: .publish),
            .mediaType(value: .image),
            .mimeType(value: "mime"),
            .custom(key: "key", value: "value")
        ]
        let queryItems = (filters as [WPRequestFilterProtocol]).makeQueryItems()
        XCTAssertEqual(queryItems[0].name, "context")
        XCTAssertEqual(queryItems[0].value, "view")
        
        XCTAssertEqual(queryItems[1].name, "page")
        XCTAssertEqual(queryItems[1].value, "1")
        
        XCTAssertEqual(queryItems[2].name, "per_page")
        XCTAssertEqual(queryItems[2].value, "1")
        
        XCTAssertEqual(queryItems[3].name, "search")
        XCTAssertEqual(queryItems[3].value, "pepe")
        
        XCTAssertEqual(queryItems[4].name, "after")
        XCTAssertEqual(queryItems[4].value, "after_1")
        
        XCTAssertEqual(queryItems[5].name, "author")
        XCTAssertEqual(queryItems[5].value, "1")
        
        XCTAssertEqual(queryItems[6].name, "author_exclude")
        XCTAssertEqual(queryItems[6].value, "1,2")
        
        XCTAssertEqual(queryItems[7].name, "before")
        XCTAssertEqual(queryItems[7].value, "before_1")
        
        XCTAssertEqual(queryItems[8].name, "exclude")
        XCTAssertEqual(queryItems[8].value, "3,4")
        
        XCTAssertEqual(queryItems[9].name, "include")
        XCTAssertEqual(queryItems[9].value, "5,6")
        
        XCTAssertEqual(queryItems[10].name, "offset")
        XCTAssertEqual(queryItems[10].value, "7")
        
        XCTAssertEqual(queryItems[11].name, "order")
        XCTAssertEqual(queryItems[11].value, "asc")
        
        XCTAssertEqual(queryItems[12].name, "orderby")
        XCTAssertEqual(queryItems[12].value, "id")
        
        XCTAssertEqual(queryItems[13].name, "parent")
        XCTAssertEqual(queryItems[13].value, "1")
        
        XCTAssertEqual(queryItems[14].name, "parent_exclude")
        XCTAssertEqual(queryItems[14].value, "2,3")
        
        XCTAssertEqual(queryItems[15].name, "slug")
        XCTAssertEqual(queryItems[15].value, "slug1")
        
        XCTAssertEqual(queryItems[16].name, "status")
        XCTAssertEqual(queryItems[16].value, "publish")
        
        XCTAssertEqual(queryItems[17].name, "media_type")
        XCTAssertEqual(queryItems[17].value, "image")
        
        XCTAssertEqual(queryItems[18].name, "mime_type")
        XCTAssertEqual(queryItems[18].value, "mime")
        
        XCTAssertEqual(queryItems[19].name, "key")
        XCTAssertEqual(queryItems[19].value, "value")
    }
    
    func testListMediaFilters_With_EmptyList() throws {
        let filters = [WPRequestFilter.ListMedia.search(value: "")]
        let queryItems = (filters as [WPRequestFilterProtocol]).makeQueryItems()
        XCTAssertTrue(queryItems.isEmpty)
    }
}
