...
final class WPPostRequestFiltersTest: XCTestCase {
    ...
    func testPostRequestField_UsingAllFields() throws {
        let fields: [WPRequestFieldTypeProtocol] = [
            ...
            WPRequestField.PostFields.featureMedia,
        ...
        XCTAssertEqual(
            queryItems!.value,
            "id,date,date_gmt,guid,link,modified,modified_gmt,slug,status,type,title,content,author,excerpt,feature_media,comment_status,ping_status,format,sticky,template,categories,tags"
        )
    }
